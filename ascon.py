...
def sifrele_aead(metin, bmetin,variant):
    ...
    associateddata = bmetin.encode()
    plaintext      = metin.encode()

    ciphertext        = ascon_encrypt(key, nonce, associateddata, plaintext,  variant)
    tag = ciphertext[-16:]

    sifreli_key = bytes_to_hex(key)
    sifreli_nonce = bytes_to_hex(nonce)
    sifreli_hex = bytes_to_hex(ciphertext)
    sifreli_tag = bytes_to_hex(tag)

    rearray = [sifreli_key,sifreli_nonce,sifreli_hex,sifreli_tag]
    return rearray



def desifrele_aead(bmetin, key, nonce, shex, variant):
    ...
    if receivedplaintext == None: print("Hata, doğrulama başarısız oldu")
    
    desifreli_metin = receivedplaintext.decode()
    #print(desifreli_metin)
